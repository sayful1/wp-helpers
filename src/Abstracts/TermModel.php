...
abstract class TermModel implements JsonSerializable {
	...
	public function get_count(): int {
		return $this->get_term()->count;
	}

	/**
	 * Get thumbnail src
	 * By default, WordPress term does not have thumbnail image. We are going to store image id to term metadata.
	 *
	...
	public function get_thumbnail_image( string $size = 'thumbnail' ) {
		$thumbnail_id = (int) $this->get_meta( '_thumbnail_id', 0 );
	...
	protected function read_meta_data() {
		foreach ( static::$meta_fields as $field ) {
			$this->meta_data[ $field['meta_key'] ] = get_term_meta( $this->get_id(), $field['meta_key'], true );
		}
