...
/**
 * Class OptionModel
 * This ORM class store data in options table rather than posts table or a custom table.
 * Only use this class for limited amount of data. e.g 99 names of Allah
 *
...
class OptionModel {
	...
	public function get_option( $data ) {
		...
		return false !== $index ? $this->prepare_item_for_response( $options[ $index ] ) : [];
	}
	...
	public function update( array $data ): array {
		...
		if ( false !== $index ) {
			$parsed_data       = wp_parse_args( $data, $options[ $index ] );
			$data              = $this->prepare_item_for_database( $parsed_data );
			$options[ $index ] = $data;
			update_option( $this->option_name, $options );
		}
