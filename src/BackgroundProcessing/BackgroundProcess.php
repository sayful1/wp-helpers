...
namespace Stackonet\WP\Framework\BackgroundProcessing;
...
abstract class BackgroundProcess extends AsyncRequest {
	 ...
	 * @return string
	 */
	protected function generate_key( int $length = 64, string $key = 'batch' ): string {
	...
	protected function time_exceeded(): bool {
		$time_limit = apply_filters( $this->identifier . '_default_time_limit', $this->default_time_limit ); // 20 seconds
		$return     = false;

		if ( time() >= ( $this->start_time + $time_limit ) ) {
			$return = true;
		}
