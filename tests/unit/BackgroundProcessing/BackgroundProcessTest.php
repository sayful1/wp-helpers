...
namespace StackonetWPFrameworkTest\BackgroundProcessing;
...
class BackgroundProcessTest extends \WP_UnitTestCase {
	/**
	 * Instance of BackgroundProcess
	 *
	 * @var MockObject|BackgroundProcess|(BackgroundProcess&MockObject)
	 */
	private $wpbp;
	...
	public function setUp(): void {
		...
		$this->wpbp = $this->getMockForAbstractClass( BackgroundProcess::class );
	...
	private function getWPBPProperty( string $name ) {
		try {
			$property = new \ReflectionProperty( BackgroundProcess::class, $name );
		} catch ( \Exception $e ) {
	...
	private function executeWPBPMethod( string $name, ...$args ) {
		try {
			$method = new \ReflectionMethod( BackgroundProcess::class, $name );
	...
	public function test_push_to_queue() {
		$this->assertClassHasAttribute( 'data', BackgroundProcess::class, 'class has data property' );
	...
	public function test_maybe_handle_cancelled() {
		...
		$this->wpbp->save();
		$this->assertCount( 2, $this->wpbp->get_batches() );
		update_site_option( $this->executeWPBPMethod( 'get_status_key' ), BackgroundProcess::STATUS_CANCELLED );
